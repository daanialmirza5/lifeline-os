...
| 4 | **T**ampering | Concurrent edits silently overwrite each other (lost update) | Optimistic concurrency on `Task.version`, both online (`ConflictError`) and offline (`SyncOperation` conflict) | Only `Task` has this protection — `CareJourney`, `CareObligation`, `AIRecommendation` do not use version checks (journey transitions are protected by the state machine instead; obligations/recommendations by their own status-guarded lifecycle, e.g. `decideRecommendation` rejects a second decision) |
...
| 6 | **I**nformation disclosure | A `CLINICIAN` views a patient outside their care team | None — authorization here is role-based, not patient-scoped (see `docs/security.md` "Known gap") | Real gap: any authenticated clinician/coordinator can read any patient's full record |
